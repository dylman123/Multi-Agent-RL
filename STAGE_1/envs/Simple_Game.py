...
'''
init_state = (0, 2, 4, 2)
env = Grid_World(width=5, height=3, initial_state=init_state, walls=walls, specials=specials)
...
AgentA = Q_Learner(agent_id=1, discount=0.3, exploit_period=100, q={}, env=env)
AgentB = Q_Learner(agent_id=2, discount=0.3, exploit_period=100, q={}, env=env)


'''
    *** MAIN PROGRAM ***
    - Agents take steps in a random order
    - 'observation' is the variable to hold state information
    - 'done' is the flag to indicate if the episode has ended
    - 'episode' stores the current episode number
    - 'AgentX.cumulative_reward' is a variable in the agent class to store the overall reward an agent has received
'''
